document.addEventListener('DOMContentLoaded', () => {
    ...
    // ==========================================
    // GitHub Stats Dynamic Fetch (Cache Bypass)
    // ==========================================
    const githubStatsImages = document.querySelectorAll('.github-stats .stat-img');
    githubStatsImages.forEach(img => {
        const originalSrc = img.getAttribute('src');
        if (originalSrc && originalSrc.includes('github-readme-stats-anuraghazra1.vercel.app') || originalSrc.includes('ghchart.rshah.org')) {
            // Append a unique timestamp parameter to bypass cache
            const separator = originalSrc.includes('?') ? '&' : '?';
            img.setAttribute('src', `${originalSrc}${separator}v=${new Date().getTime()}`);
        }
    });
